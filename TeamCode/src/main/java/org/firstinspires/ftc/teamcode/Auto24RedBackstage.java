...
@Autonomous(name="Auto24RedBackstage", group="Autonomous")
public class Auto24RedBackstage extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive() && (runtime.seconds() < 30.0) && rb.isBusy() && rf.isBusy() && lb.isBusy() && lf.isBusy()) {//sets the time limit to 30 sec and makes sure that the motors keep running(last part my not be necessary)
            ...
            if(CS.red() >= 1000)/*set number value to a value a little less than the number found in the test program*/ {//Robot scans for beacon
                Program1();//if found, go to backdrop
            }else{
                Program2();//if not found, move on
            }
            if(CS.red() >= 1000){//Robot scans for beacon
